...
public class BezierCurve
{
  ...
  // Now let's calculate all the points for the entire curve
  // from t = 0 to t = 1 with an increment of 0.01.
  public static List<Vector2> PointList2(List<Vector2> controlPoints, int width = 100, float interval = 0.01f)
  {
    int N = controlPoints.Count - 1;
    if (N > 18)
    {
    ...
    // interpolation ControlPoints
    List<Vector2> cpts = new();
    cpts.Add(controlPoints[0]);
    float interpolation = width/100;
    for (int i = 1; i < N; ++i)
    {
      cpts.Add(new Vector2(controlPoints[i].x * interpolation, controlPoints[i].y));
    }
    cpts.Add(new Vector2(controlPoints[N].x + (width - 100), controlPoints[N].y));
    ...
    for (float t = 0.0f; t <= 1.0f + interval - 0.0001f; t += interval)
    {
      ...
      {
        Vector2 bn = Bernstein(N, i, t) * cpts[i];
        p += bn;
      }
      points.Add(p);
    }
